...
## 11.5 SAP Client 接口与 Mock 实现（SAP Client インターフェースと Mock 実装）
...
## 11.6 确认服务（`src/services/confirmationService.ts`，本 Demo 的核心新增部分）（確認サービス（`src/services/confirmationService.ts`、本デモの中核となる新規追加部分））

这是回应 Part 7.3/Part 14 的关键实现：**确认状态和幂等键完全由后端生成和持有，LLM 只拿到一个不透明的 `confirmationId`。**

これは Part 7.3/Part 14 に応える重要な実装です。**確認状態と冪等キーは完全にバックエンドが生成・保持し、LLM は不透明な `confirmationId` を受け取るだけです。**
...
要点：
- `stage()` 生成 `idempotencyKey` 后就再也不会离开这个模块——`create_sales_order` 的 Tool Schema 里根本没有这个字段，模型无法伪造或搞乱它。<br>`stage()` は `idempotencyKey` を生成した後、二度とこのモジュールの外に出ることはありません——`create_sales_order` の Tool Schema にはそもそもこのフィールドが存在しないため、モデルが偽造したり混乱させたりすることはできません。
- `confirm()` 是被独立的 REST 端点调用的（`POST /confirmations/:id/confirm`，见 11.9 的 `server.ts` 示例），**完全不经过 LLM**，所以即使模型被 Prompt Injection 影响、误判用户已确认，`create_sales_order` 在真正执行前依然会因为状态还是 `PENDING` 而被拒绝。<br>`confirm()` は独立した REST エンドポイント（`POST /confirmations/:id/confirm`、11.9 の `server.ts` の例を参照）から呼び出され、**完全に LLM を経由しません**。そのため、モデルが Prompt Injection の影響を受けてユーザーが確認済みだと誤判定したとしても、`create_sales_order` は実行前に状態がまだ `PENDING` であるために拒否されます。
- `consumeForExecution()` 把"状态校验"和"标记为已执行"合并成一步，缩小了并发窗口，但**在多实例部署下这里仍然需要数据库的原子操作（如 `UPDATE ... WHERE state='CONFIRMED'` 判断受影响行数）或分布式锁**，Demo 的内存实现只在单进程、非并发场景下成立，见 11.8 的完整讨论。<br>`consumeForExecution()` は「状態検証」と「実行済みとしてマークする」を1ステップにまとめ、並行処理のウィンドウを縮小しますが、**マルチインスタンスデプロイではデータベースの原子操作（例：`UPDATE ... WHERE state='CONFIRMED'` で影響を受けた行数を判定）や分散ロックが依然として必要です**。デモのメモリ実装は単一プロセス・非並行のシナリオでのみ成立します。詳細は 11.8 の完全な議論を参照してください。
...
旧バージョンの設計との重要な違い：`createSalesOrder` は現在 `confirmationId`（および認証コンテキストから取得した `userId`）のみを受け付け、業務パラメータと冪等キーはいずれも `confirmationService.consumeForExecution()` から引き出されます。**Tool 層と LLM はこの両者の受け渡しに一切関与しません。**

## 11.8 幂等存储（`src/services/idempotencyStore.ts`，Demo 用内存实现）（冪等ストレージ（`src/services/idempotencyStore.ts`、デモではメモリ実装を使用））
...
>
> ⚠️ **この `Map` 実装は教育デモ用のみで、並行時の冪等性を保証しません**：もし2つのリクエストがほぼ同時に同じ `idempotencyKey` で `markPending`/`get` を呼び出した場合、`Map` の読み取り-判定-書き込みは原子操作ではないため、両方のリクエストが `get()` でまだ `COMPLETED` 状態を見ていない間に両方とも先へ進んでしまい、並行二重書き込みが発生する可能性があります。Part 11.6 の `confirmationService.consumeForExecution()` は「確認レコードを一度だけ消費する」ことでこのウィンドウを縮小していますが、真に本番レベルの保証を実現するにはなお以下が必要です。
> 1. 原子操作に対応したストレージ（Redis の `SETNX`/Lua スクリプト、またはデータベースのユニークインデックス + トランザクション）でこの `Map` を置き換える。
> 2. 冪等キーにユニーク制約を追加し、並行する2回目の書き込み操作が「空値を読んだのでそのまま進む」のではなく、ユニークキーの衝突により直接失敗するようにする。
>
> このデモ実装の挙動を「並行時の冪等性問題はすでに解決済み」と誤解しないでください——これはあくまでインターフェースの形を先に固定し、実際のストレージに置き換える際に呼び出し側のコードを変更しなくて済むようにしているだけです（Part 13.2 を参照）。

## 11.9 Tool 执行、Agent Loop 与确认按钮的 REST 接口（Tool 実行、Agent Loop と確認ボタンの REST インターフェース）
...
要点：
- `loopGuard` 防止模型陷入无限工具调用循环。<br>`loopGuard` はモデルが無限のツール呼び出しループに陥るのを防ぎます。
- `correlationId` 贯穿整个调用链（详见 Part 15），从 HTTP 入口生成，传递到每一次工具调用和日志记录。<br>`correlationId` は呼び出しチェーン全体を貫通し（詳細は Part 15 を参照）、HTTP 入口で生成され、すべてのツール呼び出しとログ記録に渡されます。
- `userId` 来自认证中间件（Part 9.5 的身份传递机制），而不是请求体或模型参数——这样 `confirmationService` 才能可靠地校验"确认这个订单的人和现在要执行创建的人是不是同一个人"。<br>`userId` は認証ミドルウェア（Part 9.5 のアイデンティティ伝播メカニズム）から取得され、リクエストボディやモデルのパラメータではありません——こうすることで `confirmationService` は「この受注を確認した人と、今作成を実行しようとしている人が同一人物かどうか」を確実に検証できます。
- 确认按钮的 REST 接口和 Agent Loop 是两条独立的请求路径，唯一的桥梁是 `confirmationId` 这个不透明字符串，这正是把"是否确认"从 LLM 的自我声明里剥离出去的关键结构。<br>確認ボタンの REST インターフェースと Agent Loop は2つの独立したリクエストパスであり、唯一の橋渡しは `confirmationId` というこの不透明な文字列です。これこそが「確認済みかどうか」を LLM の自己申告から切り離す重要な構造です。

## 11.10 Test Cases（テストケース）
...
このテストケースは本節で強調したアーキテクチャ上のポイントに直接対応しています。**未確認では実行不可、他ユーザーでは実行不可、重複実行では重複作成不可**、さらに Part 13 で一貫して求めているエラー分類テスト（業務エラー vs 状態不確定）も含まれます。

## 11.11 从 Mock 到真实 SAP Sandbox 的替换路径（Mock から実際の SAP Sandbox への置き換え手順）

1. 保持 `SapClient` 接口不变（新增的 `simulateSalesOrder` 也一并实现）。<br>`SapClient` インターフェースを変更しない（新規追加の `simulateSalesOrder` も併せて実装する）。
2. 新建 `src/sap/odataSapClient.ts`，实现同一接口，内部改为真实 OData HTTP 调用（含 CSRF Token 获取、Destination 解析、Sales Order Simulation API 调用）。<br>`src/sap/odataSapClient.ts` を新規作成し、同一インターフェースを実装する。内部は実際の OData HTTP 呼び出しに変更する（CSRF Token 取得、Destination 解析、Sales Order Simulation API 呼び出しを含む）。
3. 通过环境变量/配置切换注入哪个实现。<br>環境変数・設定の切り替えによって、どちらの実装を注入するかを制御する。
4. `confirmationService.ts`、`salesOrderService.ts`、`agentLoop.ts`、Tool 定义、测试用例（针对 Business Service 层的部分）**完全不需要修改**——这正是 Part 6.5 提到的"Tool Abstraction Layer + Adapter 模式"的实际收益体现。<br>`confirmationService.ts`、`salesOrderService.ts`、`agentLoop.ts`、Tool 定義、テストケース（Business Service 層に関する部分）は**一切変更不要**です——これはまさに Part 6.5 で述べた「Tool Abstraction Layer + Adapter パターン」の実際の効果の表れです。
5. 生产化时把 `confirmationService` 和 `idempotencyStore` 的内存 `Map` 换成 Redis/数据库表，并按 11.8 的说明加上原子操作/唯一约束。<br>本番化する際は `confirmationService` と `idempotencyStore` のメモリ内 `Map` を Redis/データベーステーブルに置き換え、11.8 の説明に従って原子操作・ユニーク制約を追加する。

## 11.12 项目中你需要记住什么（プロジェクトで覚えておくべきこと）

- 写操作的"确认"不能只靠 Prompt 里的文字要求，必须由后端一条可查询、可校验的状态记录来判断——这是本 Demo 相比"只在 System Prompt 里写规则"的关键改进。<br>書き込み操作の「確認」は Prompt 内の文言による要求だけに頼ることはできず、バックエンドの照会・検証可能な状態レコードによって判断されなければなりません——これが本デモの「System Prompt にルールを書くだけ」に比べた重要な改善点です。
- 幂等键应该在用户确认时由**后端**生成并持有，不要把它做成 LLM 需要感知、生成或传递的参数；`create_sales_order` 面向模型的接口应该尽量收窄到"只传一个不透明的确认 ID"。<br>冪等キーはユーザー確認時に**バックエンド**が生成・保持すべきであり、LLM が認識・生成・伝達する必要のあるパラメータにしてはいけません。`create_sales_order` のモデル向けインターフェースはできる限り「不透明な確認 ID を一つだけ渡す」形に絞り込むべきです。
- Mock 阶段就要模拟"偶发超时"这种边界情况，否则替换真实 SAP 后你会第一次遇到"状态不确定"问题时手忙脚乱。<br>Mock 段階から「偶発的なタイムアウト」のような境界ケースをシミュレートしておくべきです。そうしないと、実際の SAP に置き換えた後に初めて「状態不確定」問題に遭遇した際に慌てることになります。
- Demo 里用内存 `Map` 实现的确认记录和幂等存储，只用于讲解接口形状，**不具备并发幂等保证**，生产环境必须换成支持原子操作的存储。<br>デモ内のメモリ `Map` で実装された確認レコードと冪等ストレージは、インターフェースの形を説明するためだけのものであり、**並行時の冪等性を保証しません**。本番環境では必ず原子操作をサポートするストレージに置き換える必要があります。
- 测试用例要覆盖：未确认拒绝执行、跨用户拒绝执行、重复调用不重复创建、业务错误（客户冻结）、系统性错误（超时→状态未知），这五类是 WRITE Tool 测试的最低要求。<br>テストケースは、未確認では実行拒否、他ユーザーでは実行拒否、重複呼び出しでは重複作成なし、業務エラー（顧客ブロック）、システム的エラー（タイムアウト→状態不明）の5種類をカバーする必要があります。これらは WRITE Tool テストの最低要件です。
