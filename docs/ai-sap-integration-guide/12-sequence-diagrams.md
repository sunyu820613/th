...
- 时序图要体现"歧义必须追问"（第一张图里的 search_customer 返回多个候选）而不是省略这一步。
- 审批分支是一个独立的异步等待状态，Agent Backend 需要有机制在审批完成后"回来"继续执行（Webhook 回调或轮询），而不是让用户的对话 Session 一直同步阻塞等待。
- 超时分支的核心不是"重试"，而是"用另一种方式核实真实状态"，这是幂等设计里最容易被忽视但最重要的一环（详见 Part 13）。
