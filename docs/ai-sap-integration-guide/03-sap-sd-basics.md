...
| 25 | 信用检查 | Credit Check | 判断客户信用额度是否够支撑本次订单——注意结果不一定是"拒绝创建"，很多配置下是"订单照常创建，但打上 Credit Block 冻结后续处理"，需信用管理员 Release/Reject | AI 转述结果时必须以 SAP 返回的凭证号+信用状态字段为准，不能把"检查未通过"简单等价于"订单未创建"（详见 Part 2 第四步、Part 13 错误场景表） | 超额度可能订单被创建但 Block，也可能直接被拒绝，取决于 Customizing |
